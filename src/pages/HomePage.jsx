...
function ResultsPanel({ analyzing, results, text, onApply, onApplyAll }) {
  ...
  // ── Download report — saves full report as a .txt file ───────────────────
  const downloadReport = () => {
    if (!results) return
    const date  = new Date().toLocaleString()
    const lines = results.detected.length > 0
      ? results.detected.map((d, i) => `  ${i + 1}. "${d.word}" → "${d.suggestion}"\n     Reason: ${d.reason}`).join('\n')
      : '  No biased patterns detected.'
    const body = [
      '══════════════════════════════════════════',
      '           GENTEK ANALYSIS REPORT         ',
      '══════════════════════════════════════════',
      `Date:           ${date}`,
      `Classification: ${results.label}`,
      `Bias Score:     ${results.score}%`,
      `Words analyzed: ${results.words}`,
      `Patterns found: ${results.detected.length}`,
      '',
      '── SUGGESTIONS ────────────────────────────',
      lines,
      '',
      ...
      '',
      '══════════════════════════════════════════',
      'Generated by GENTEK · gentek.app',
    ].join('\n')
    const blob = new Blob([body], { type: 'text/plain' })
    const url  = URL.createObjectURL(blob)
    const a    = document.createElement('a')
    a.href     = url
    a.download = `gentek-report-${Date.now()}.txt`
    a.click()
