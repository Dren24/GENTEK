...
export default function AppSidebar() {
  ...
  const loadText  = (item)         => go('/', { loadText: item.text })
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 overflow-y-auto overflow-x-visible py-2 editor-scroll ${!sidebarOpen ? 'overflow-visible' : ''}`}>
        ...
        {/* ── COLLAPSED: icon rail with popups ── */}
        {!sidebarOpen && (
      ...
      <div className={`border-t border-gray-200 dark:border-gray-800 flex-shrink-0 ${sidebarOpen ? 'px-2 py-2 space-y-0.5' : 'px-1.5 py-2 space-y-0.5'}`}>
        ...
        {/* Settings & Help — open/close in expanded; in footer popup when collapsed */}
        {sidebarOpen ? (
          <>
            {[{ icon: Gear, label: 'Settings', path: '/settings' }, { icon: Question, label: 'Help', path: '/about' }].map(({ icon: Icon, label, path }) => (
              <button key={label} onClick={() => go(path)} className="w-full flex items-center gap-2.5 px-3 py-2 text-gray-500 dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 hover:bg-white dark:hover:bg-gray-800 rounded-xl transition-colors text-xs font-medium">
                <Icon size={16} className="flex-shrink-0" />
                {label}
              </button>
            ))}
            <button onClick={logout} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors text-xs font-medium">
          ...
          <>
            {[{ icon: Gear, label: 'Settings', path: '/settings' }, { icon: Question, label: 'Help', path: '/about' }].map(({ icon: Icon, label, path }) => (
              <button key={label} title={label} onClick={() => go(path)} className="w-full flex justify-center p-2.5 rounded-xl text-gray-500 dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 hover:bg-white dark:hover:bg-gray-800 transition-colors">
                <Icon size={16} />
              </button>
            ))}

            {/* User avatar — shows popup */}
